...
void initMaze() {
    for (int i = 0; i < MAZE_SIZE; i++) {
        for (int j = 0; j < MAZE_SIZE; j++) {
            ...
            if (j == MAZE_SIZE - 1) walls[i][j][NORTH] = true;
            if (i == MAZE_SIZE - 1) walls[i][j][EAST] = true;
            if (j == 0) walls[i][j][SOUTH] = true;
            if (i == 0) walls[i][j][WEST] = true;
        }
    }
}
...
void updateWalls() {
    ...
    if (API::wallFront()) {
        ...
        int nx = x, ny = y;
        if (orient == NORTH) ny++;
        else if (orient == EAST) nx++;
        else if (orient == SOUTH) ny--;
        else if (orient == WEST) nx--;
    ...
    if (API::wallRight()) {
        ...
        walls[x][y][dir] = true;
        int nx = x, ny = y;
        if (dir == NORTH) ny++; else if (dir == EAST) nx++; else if (dir == SOUTH) ny--; else if (dir == WEST) nx--;
        if (nx >= 0 && nx < MAZE_SIZE && ny >= 0 && ny < MAZE_SIZE) walls[nx][ny][(dir + 2) % 4] = true;
    }
...
void floodFill() {
    ...
    flood[7][7] = 0; q.push({7, 7});
    flood[7][8] = 0; q.push({7, 8});
    flood[8][7] = 0; q.push({8, 7});
    flood[8][8] = 0; q.push({8, 8});
...
void moveNext() {
    ...
    if (!walls[x][y][NORTH] && y < MAZE_SIZE-1) {
        if (flood[x][y+1] < minVal) { minVal = flood[x][y+1]; bestDir = NORTH; }
    }
    // Check East
    if (!walls[x][y][EAST] && x < MAZE_SIZE-1) {
        if (flood[x+1][y] < minVal) { minVal = flood[x+1][y]; bestDir = EAST; }
    }
    // Check South
    if (!walls[x][y][SOUTH] && y > 0) {
        if (flood[x][y-1] < minVal) { minVal = flood[x][y-1]; bestDir = SOUTH; }
    }
    // Check West
    if (!walls[x][y][WEST] && x > 0) {
        if (flood[x-1][y] < minVal) { minVal = flood[x-1][y]; bestDir = WEST; }
    }
    ...
    if (bestDir != -1) {
        ...
        // Update coordinate
        if (orient == NORTH) y++;
        else if (orient == EAST) x++;
        else if (orient == SOUTH) y--;
        else if (orient == WEST) x--;
    }
}
...
std::vector<Coord> main_path;
x=0;
y=0;
orient=NORTH;
main_path.push_back({x,y});
...
for (int i = main_path.size() - 1; i >= 0; i--) {
    std::cerr << "(" << main_path[i].x << "," << main_path[i].y << ") ";
}
std::cerr << std::endl;
